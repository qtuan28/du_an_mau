...
class pickleballController {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($username) && !empty($password) && !empty($email)) {
                ...
                if (!empty($sdt) && !preg_match('/^[0-9]{9,11}$/', $sdt)) {
                    $error = "Số điện thoại không hợp lệ! Vui lòng nhập 9-11 chữ số.";
                    require_once 'views/register.php';
                    return;
                }
    ...
    public function postThanhToan() {
        ...
        // Lấy thông tin từ form POST (có thể điền sẵn từ session)
        $hoTen   = trim($_POST['ho_ten']  ?? ($_SESSION['user']['username'] ?? 'Khách hàng'));
        ...
        foreach ($gioHang as $item) {
            ...
            // Trừ số lượng kho và ghi lịch sử
            $sanPhamModel->updateStock($item['product_id'], -$item['so_luong'], 'ban_hang', "Đơn hàng $maDonHang");
        }
    ...
    public function adminXoaDanhMuc()
    {
        ...
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $danhMucModel = new DanhMuc();
            if ($danhMucModel->hasProducts($id)) {
    ...
    // Lịch sử tồn kho của một sản phẩm
    ...
    public function adminThemSanPham() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($productId > 0 && $so_luong_nhap > 0) {
                $sanPhamModel->updateStock($productId, $so_luong_nhap, 'nhap_hang', 'Nhập kho ban đầu');
            }
    ...
    public function adminSuaSanPham() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($so_luong_nhap > 0) {
                $sanPhamModel->updateStock($id, $so_luong_nhap, 'nhap_hang', 'Nhập kho thêm');
            }
